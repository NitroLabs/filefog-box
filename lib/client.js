...
Client.prototype.searchFiles = function (filters,options) {
    ...
    // String to match against filenames/content
    // XXX: Will not work for listing files with unicode names!
    params.query = "abcdefghijklmnopqrstuvwxyz".split("").join(" OR "); 
    params.scope = "user_content";
    if (options.max_results){
        params.limit = options.max_results;
    }
    // The page_token is actually the offset for the next search
    ...
    return self._getClient().then(function (client) {
        var deferred = Q.defer();
        var formattedUrl = url.format({pathname:'https://api.box.com/2.0/search/',query:params});
        
        client.get({url:formattedUrl},
            function (err, r, body) {
                err = errorHandler(r, err);
                if (err) return deferred.reject(err);
                return deferred.resolve(JSON.parse(body));
